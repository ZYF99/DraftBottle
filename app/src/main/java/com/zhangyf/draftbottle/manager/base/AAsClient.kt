...
class AAsClient private constructor(val retrofit: Retrofit, val okHttpClient: OkHttpClient) {
	...
	class Builder(
		...
		fun build(baseUrl: String = BuildConfig.AASURL): AAsClient {
			...
			return AAsClient(retrofit, client)
		}
	}
	
	companion object {
		val defaultClient: AAsClient
			get() = Builder().build()
	}
}
